...
def calculate_scores(row, use_gst, use_upi, use_aa, use_epfo, use_invoice):
    ...
    if use_aa:
        cashflow_score = max(0, min(100, 96 - row["cashflow_volatility"] * 125 + min(18, row["avg_bank_balance"] / 25000)))
        repayment_score = max(0, min(100, 100 - row["bounce_count_6m"] * 7.5 - row["repayment_delay_days"] * 1.7))
        scores["Cash-flow Stability"] = round(cashflow_score, 1)
        scores["Repayment Discipline"] = round(repayment_score, 1)

    if use_upi:
        upi_score = max(0, min(100, 52 + row["upi_growth_rate"] * 170 + min(32, row["upi_monthly_value"] / 45000)))
        scores["UPI Strength"] = round(upi_score, 1)

    if use_gst:
        gst_score = max(0, min(100, row["gst_filing_consistency"] * 100 + min(10, row["revenue_growth_rate"] * 20)))
        growth_score = max(0, min(100, 55 + row["revenue_growth_rate"] * 180))
        scores["GST Consistency"] = round(gst_score, 1)
        scores["Business Growth"] = round(growth_score, 1)

    if use_invoice:
        invoice_score = max(0, min(100, row["invoice_paid_ratio"] * 100))
        scores["Invoice Reliability"] = round(invoice_score, 1)

    if use_epfo:
        payroll_score = max(0, min(100, row["payroll_regularity"] * 100 + min(10, row["employee_count"] / 2)))
...
def decision_from_score(score, loan_amount, row):
    ...
    exposure_ratio = (loan_amount + row["existing_credit_exposure"]) / max(1, row["gst_monthly_sales"] * 6)
    ...
    if adjusted_score >= 76:
        return "Approve", "Low Risk", adjusted_score, "Approve working capital loan with monthly portfolio monitoring."
    elif adjusted_score >= 58:
        return "Manual Review", "Medium Risk", adjusted_score, "Request additional verification or offer a lower-ticket credit line."
    else:
        return "Do Not Auto-Approve", "High Risk", adjusted_score, "Do not approve instantly. Monitor alternate data signals and reassess."
...
def generate_repayment_details(row):
    ...
    if bounce_count == 0 and delay_days == 0:
        return pd.DataFrame([{"Month": "All Months", "Bounce Event": "No", "Delay Days": 0, "Observation": "Clean repayment behavior"}])
    ...
    for m in months:
        if m in flagged_months:
            ...
            obs = []
            if bounce == "Yes":
                obs.append("EMI/repayment bounce recorded")
            if delay > 0:
                obs.append(f"Payment delayed by {delay} days")
            if not obs:
                obs.append("Minor repayment variation")
            events.append({"Month": m, "Bounce Event": bounce, "Delay Days": delay, "Observation": "; ".join(obs)})
        else:
            events.append({"Month": m, "Bounce Event": "No", "Delay Days": 0, "Observation": "No adverse repayment event"})
...
def generate_gst_details(row):
    ...
    for i, m in enumerate(months):
        ...
        out.append({"Month": m, "GST-like Sales": sales[i], "Filing Status": status})
...
def generate_growth_details(row):
    ...
    for m, s in zip(months, sales):
        if prev is None:
            ...
            mom = ((s - prev) / prev) * 100 if prev else 0.0
        data.append({"Month": m, "GST-like Sales": s, "MoM Growth %": round(mom, 1)})
        prev = s
...
st.caption("POC uses synthetic data to simulate GST, UPI, AA, EPFO, invoice and repayment signals.")
...
with st.sidebar:
    ...
    selected_business = st.selectbox("Select MSME Applicant", df["business_name"].tolist())
    ...
    loan_amount = st.slider("Requested Loan Amount", 50000, 2000000, 500000, step=50000)
    tenure = st.slider("Tenure in Months", 3, 36, 12, step=3)

scores, overall_score = calculate_scores(row, use_gst, use_upi, use_aa, use_epfo, use_invoice)
decision, risk_category, adjusted_score, recommendation = decision_from_score(overall_score, loan_amount, row)
...
with tabs[0]:
    ...
    if decision == "Approve":
        st.markdown(f'<div class="decision-approve">Decision: {decision} — {recommendation}</div>', unsafe_allow_html=True)
    elif decision == "Manual Review":
        st.markdown(f'<div class="decision-review">Decision: {decision} — {recommendation}</div>', unsafe_allow_html=True)
    else:
        st.markdown(f'<div class="decision-reject">Decision: {decision} — {recommendation}</div>', unsafe_allow_html=True)
...
with tabs[1]:
    ...
    if scores:
        score_df = pd.DataFrame({"Dimension": list(scores.keys()), "Score": [round(v, 1) for v in scores.values()]})

        col_a, col_b = st.columns([1, 1])
        with col_a:
            fig = px.bar(score_df, x="Score", y="Dimension", orientation="h", range_x=[0,100], title="Score Breakdown")
        ...
        with s2:
            ...
            if risks:
                for dim, msg in risks:
                    st.markdown(f'<div class="risk-tag">⚠️ {dim}: {msg}</div>', unsafe_allow_html=True)
                    ...
                    if dim == "GST Consistency":
                        with st.expander("More info: GST / compliance details"):
                            ...
                            fig_gst = px.bar(gst_df, x="Month", y="GST-like Sales", color="Filing Status", title="Monthly GST-like Sales and Filing Status")
                        ...
                        with st.expander("More info: repayment bounce / delay details"):
                            repay_df = generate_repayment_details(row)
                            st.dataframe(repay_df, use_container_width=True)

                    elif dim == "Business Growth":
                        with st.expander("More info: monthly growth details"):
                            growth_df = generate_growth_details(row)
                            st.dataframe(growth_df, use_container_width=True)
                            fig_growth = px.line(growth_df, x="Month", y="GST-like Sales", markers=True, title="Monthly Business Sales Trend")
                        ...
                        with st.expander("More info: bank balance trend"):
                            ...
                            fig_balance = px.line(balance_df, x="Month", y="Average Balance", markers=True, title="Average Balance Trend")
                        ...
                        with st.expander("More info: UPI collection trend"):
                            ...
                            fig_upi = px.line(upi_df, x="Month", y="UPI Collections", markers=True, title="UPI Collection Trend")
                        ...
                        with st.expander("More info: payroll details"):
                            payroll_df = pd.DataFrame([{
                                ...
                                "Payroll Regularity %": round(row["payroll_regularity"] * 100, 1),
                                "Estimated Monthly Obligation": int(row["monthly_obligation"])
                            }])
                        ...
                        with st.expander("More info: invoice payment details"):
                            invoice_df = pd.DataFrame([{
                                "Invoice Paid Ratio %": round(row["invoice_paid_ratio"] * 100, 1),
                                "Collection Strength": "Good" if row["invoice_paid_ratio"] >= 0.75 else "Moderate/Weak"
                            }])
...
with tabs[2]:
    ...
    fig_line = px.line(trend_df, x="Month", y=["GST-like Sales", "UPI Collections", "Average Bank Balance"],
                       markers=True, title="Six-Month Alternate Data Trend")
...
with tabs[3]:
    ...
    fig = px.line(simulator_df, x="Loan Amount", y="Adjusted Score", markers=True, title="Loan Amount vs Adjusted Score")
    st.plotly_chart(fig, use_container_width=True)
...
with tabs[4]:
    ...
    st.markdown("""
    This POC does not connect to real banking APIs. It simulates how the production solution would fit into India's digital lending ecosystem.
    """)
    ...
    **Production integration concept:**
    ...
    3. CrediLens scoring engine computes multidimensional health score.
